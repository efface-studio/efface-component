...
export interface LogoScene3DProps {
  ...
  transparent?: boolean
  /** 마크를 가운데 놓는다. 히어로 기본은 오른쪽으로 치우친다. */
  centered?: boolean
}
 ...
 */
export function LogoScene3D({ className, still, transparent = false, centered = false }: LogoScene3DProps) {
  ...
  useEffect(() => {
    ...
    import('@/components/three/glassScene').then(({ createGlassScene }) => {
      ...
      dispose = createGlassScene(canvasRef.current, { still: frozen, transparent, centered })
    })
    return () => {
      cancelled = true
      dispose?.()
    }
  }, [frozen, transparent, centered])
