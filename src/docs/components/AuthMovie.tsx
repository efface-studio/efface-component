  ...
  return (
    <div ref={host} className={cn('relative flex h-full w-full items-center justify-center overflow-hidden bg-[#050813]', className)}>
      <div ref={stage} className="relative shrink-0 origin-center overflow-hidden" style={{ width: W, height: H, transform: `scale(${scale})` }} data-theme="dark">
        ...
        {/* 메뉴 막대 — Tahoe: 투명, 글자만 */}
        <div className="absolute inset-x-0 top-0 flex h-9 items-center justify-between px-4 text-[13.5px] font-medium text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.35)]">
          ...
          <span className="flex items-center gap-3.5 text-[13px]">
            <span className="inline-flex h-4 w-4 items-center justify-center rounded-[3px] bg-white/90 text-[9px] font-bold text-[#1d3ea8]">♪</span>
            <Wifi size={16} strokeWidth={2.2} />
            <span className="flex items-center gap-1">
              <Battery size={20} strokeWidth={2} /> <span className="text-[12px]">100%</span>
            </span>
            <span className="inline-flex h-4 w-6 items-center justify-center rounded-[4px] border border-white/80">
            ...
            >
              ...
              <div className="grid flex-1 grid-cols-5 content-start gap-x-4 gap-y-7 p-8">
                {APPS_GRID.map(([id, label]) => (
                  <div key={id} data-tour={`app-${id}`} className="flex flex-col items-center gap-2">
                    <motion.span animate={id === 'efface' && pressed ? { scale: 0.9 } : { scale: 1 }} transition={{ duration: 0.12 }} className="flex">
        ...
        <div className="absolute inset-x-0 bottom-2.5 flex justify-center">
          <div className="liquid-glass flex items-end gap-1.5 rounded-[26px] px-3 py-2">
            {DOCK.map(([id, label]) => (
              <motion.div
                key={id}
                data-tour={`dock-${id}`}
                ...
                animate={hoverDock === id ? { y: -10, scale: 1.18 } : { y: 0, scale: 1 }}
                transition={{ type: 'spring', stiffness: 380, damping: 22 }}
              >
                <AppIcon id={id} size={54} />
                {(id === 'finder' || (id === 'apps' && scene === 'apps')) && <span className="absolute -bottom-1.5 h-1 w-1 rounded-full bg-white/85" />}
              </motion.div>
            ))}
            <span className="mx-1.5 h-12 w-px self-center bg-white/25" />
            <motion.div className="relative flex flex-col items-center" animate={windowOpen ? { y: 0 } : { y: 0 }}>
              <AppIcon id="efface" size={54} />
              {windowOpen && <span className="absolute -bottom-1.5 h-1 w-1 rounded-full bg-white/85" />}
            </motion.div>
          </div>
        </div>
