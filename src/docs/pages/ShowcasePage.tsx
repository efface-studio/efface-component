  ...
  return (
    <DocPage eyebrow="showcase" title="Showcase" lead="전부 스스로 돌아가요 — 가짜 커서가 떠다니거나 글자가 저절로 쳐져요. 마우스를 올리면 멈추고 직접 해볼 수 있고, 나가면 잠시 뒤 다시 돌아요.">
      ...
      <div className="grid gap-5 md:grid-cols-2">
          <Card title="MobileTabBars" desc="실제 iPhone 16 Pro 목업 안의 탭 바 다섯 가지 — iOS 26 리퀴드 글래스(떠 있는 알약 + 따로 떨어진 검색 원, 유리 렌즈가 미끄러져요), iOS 18 표준 탭 바(선택하면 심볼이 튀고 배지가 지워져요), 가운데 액션(둥근 홈의 버튼이 ×로 돌며 미니 액션을 펼쳐요), 구이(파란 물방울이 늘어났다 끊기며 옮겨가요), 모프(밑줄이 원으로 부풀어요). 탭을 누르면 홈·탐색·검색·알림·나 화면이 실제로 바뀌고, 폰마다 번갈아 누르며 돌다가 마우스를 올리면 멈춰 직접 눌러볼 수 있어요." tag="new" dark className="md:col-span-2">
            <MobileTabBars />
          </Card>
